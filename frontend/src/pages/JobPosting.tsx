      ...
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: easeOut }}>
          <div className="flex items-center justify-between mb-6">
            ...
            <Button className="bg-white text-surface-950 hover:bg-surface-100" onClick={() => setIsModalOpen(true)}><Plus className="h-4 w-4" />Post Job</Button>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="bg-surface-900 border border-surface-800 rounded-3xl p-12 text-center shadow-float">
              ...
              <Button className="bg-white text-surface-950 hover:bg-surface-100" onClick={() => setIsModalOpen(true)}><Plus className="h-4 w-4" />Post Your First Job</Button>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-3 text-xs text-surface-400 font-medium">
        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Post a New Job">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input label="Required Skill" placeholder="e.g. Plumber" value={form.skillRequired} onChange={e => setForm(p => ({ ...p, skillRequired: e.target.value }))} required className="bg-surface-900 border-surface-800 text-white placeholder:text-surface-600 focus:border-accent/50 focus:ring-accent/10" labelClassName="text-surface-300" />
            ...
            <Button type="submit" isLoading={isSaving} className="w-full bg-white text-surface-950 hover:bg-surface-100" size="lg">Post Job</Button>
          </form>
        </Modal>
      </div>
    </div>
